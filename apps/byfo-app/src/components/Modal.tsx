...
export default function Modal(props) {
  ...
  return (
    <div
      class={
        props.modalName
          ? `fixed flex top-0 left-0 justify-center items-center w-screen h-screen p-0 m-0 bg-black bg-opacity-40`
          : ""
      }
      hidden={!props.modalName}
    >
      {props.modalName ? (
        <article class="relative bg-app border border-gray-500 w-full max-w-lg h-fit max-h-lg p-8 rounded-lg flex flex-col items-center justify-between gap-4">
          <button
            class="absolute top-0 right-0 h-6 w-6 text-lg m-2 mt-1 align-bottom"
            onClick={closeModal}
          >
            x
          </button>
          ...
          <button
            class="h-12 w-24 p-2 disabled:bg-gray-400 rounded-md text-white text-lg bg-brand-primary"
            disabled={props.verifyInputs ? buttonDisabled() : false}
          >
            Host
          </button>
        </article>
