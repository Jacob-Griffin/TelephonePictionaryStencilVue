...
export async function submitRound(gameid,name,round,content,staticRoundInfo){
  let savedContent = {contentType:content.contentType};
  if(content.contentType == 'image'){
    savedContent.content = await uploadImage(gameid,name,round,content.content);
  }
  else{
...
export async function finalizeGame(gameid){
  ...
  storeGame(gameid,finalStackData)
  .then(success => {
