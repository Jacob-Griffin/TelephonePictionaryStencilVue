...
test('sortNamesBy', ()=>{
    ...
    expect(src.sortNamesBy([{name:'a', id: 'd'},{name:'c', id:'e'},{name:'b',id: 'f'}], 'id')).toEqual([{name:'a', id: 'd'},{name:'c', id:'e'},{name:'b',id: 'f'}]);
    expect(()=>src.sortNamesBy([{name:'a', id: 'd'},{name:'c', id:'e'},{name:'b',id: 2}], 'id')).toThrowError('Unexpected non-string value used as name');
    expect(src.sortNamesBy([{name:'a'},{name:'C'},{name:'b'}], 'name')).toEqual([{name:'a'},{name:'b'},{name:'C'}]);
...
test('username', ()=>{
    ...
    expect(src.validUsername(`Jacob
on
multiple
lines`)).toBe(true);
